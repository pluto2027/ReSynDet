...
# Modified to attach GIQA metadata to the unlabeled-data pipeline.
import logging
import numpy as np
import operator
import json
...
from giqa_ssod.data.common import (
...
def build_detection_semisup_train_loader_two_crops(cfg, mapper=None):
    ...
    if mapper is None:
        mapper = DatasetMapper(cfg, True)
    if hasattr(mapper, "filter_unlabeled_dataset_by_iqa"):
        unlabel_dicts = mapper.filter_unlabeled_dataset_by_iqa(unlabel_dicts)
    if hasattr(mapper, "set_iqa_normalization_from_dataset"):
        mapper.set_iqa_normalization_from_dataset(unlabel_dicts)

    for d in label_dicts:
        d["is_unlabeled"] = False
    for d in unlabel_dicts:
        d["is_unlabeled"] = True
