...
# Modified to support per-image weighting of ROI losses.
import torch
...
from giqa_ssod.modeling.roi_heads.fast_rcnn import FastRCNNFocaltLossOutputLayers
    ...
    def _forward_box(
        ...
        if (
            ...
            if not use_iqa_weights:
                ...
                num_props_per_img = [len(p) for p in proposals]
                split_scores = scores.split(num_props_per_img, dim=0)
                split_deltas = proposal_deltas.split(num_props_per_img, dim=0)

                loss_sums: Dict[str, torch.Tensor] = {}
                num_images = max(1, len(proposals))

                for idx, (score_i, delta_i, prop_i) in enumerate(
                    zip(split_scores, split_deltas, proposals)
                ):
                    weight = float(iqa_weights[idx])
                    losses_i = self.box_predictor.losses((score_i, delta_i), [prop_i])
